...
if (Meteor.isServer){
    ...
    describe('Action Publication', function (){
        ...
        it('actions published correctly', function (done){
            ...
            RetroActions.insert(TestData.fakeRetroAction({ status: Constants.RetroItemStatuses.COMPLETE, completedAt: new Date() }))
            RetroActions.insert(TestData.fakeRetroAction({ status: Constants.RetroItemStatuses.COMPLETE, completedAt: expired.toDate() }))
